...
## 2.2.2 Assembly and Genome Polishing
...
Let's have quick look at the àssembly_info.txt` file and attempt to explain it's contents.
...
:::::{.callout-important icon=false}
## ***Exercise:*** Obtain statistics from `assembly.fasta` file.
...
Before we move on to the next step, let's rename our assembly (`assembly.fasta`) to something more meaningful:

```bash
mv assembly.fasta barcode30_assembly.fasta
```
...
:::{.callout}
```bash
medaka_consensus -i barcode30_assembly.fasta -d ${DRAFT} -o barcode30_medaka -t 8 -m r941_min_high_g303
```
```
...
## 2.2.3 Assembly quality assessment with [`QUAST`](https://quast.sourceforge.net/)
